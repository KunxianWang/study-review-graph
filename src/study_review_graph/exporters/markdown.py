...
def export_markdown_bundle(state: StudyGraphState) -> dict[str, str]:
    ...
    Path(output_paths["formulas"]).write_text(_render_formulas(state.formulas), encoding="utf-8")
...
def _render_formulas(formulas: list[FormulaArtifact]) -> str:
    if not formulas:
        return "# Formula Catalog\n\nNo formulas extracted.\n"

    sections = ["# Formula Catalog\n"]
